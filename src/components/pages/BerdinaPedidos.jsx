...
const ESTADOS   = ['En analisis', 'Pendiente', 'En proceso', 'Completado', 'Cancelado']
...
export default function BerdinaPedidos() {
  ...
  const exportarExcel = () => {
    ...
    const headers  = ['N° Pedido', 'Fecha', 'C.C.', 'Repuesto', 'Cant.', 'Descripción', 'Urgencia', 'Grupo', 'Estado']
    ...
    const aoa = [
      ...
      ...lista.map(item => [
        ...
        item.urgencia,
        item.grupo,
        item.estado === 'Pedido' ? 'En analisis' : (item.estado || ''),
      ]),
    ]

    const ws = XLSX.utils.aoa_to_sheet(aoa)
    ws['!merges'] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: headers.length - 1 } }]
    ws['!cols'] = [8, 10, 8, 22, 6, 30, 10, 14, 12].map(w => ({ wch: w }))
  ...
  const verDetalle = (item) => {
    const filas = item._items.map(i =>
      `<tr>
        ...
        <td>${i.grupo}</td>
        <td>${i.descripcion || '—'}</td>
        <td>${i.estado === 'Pedido' ? 'En analisis' : (i.estado || '—')}</td>
      </tr>`
    ).join('')
    Swal.fire({
      ...
      html: `<div style="overflow-x:auto">
        <table class="table table-sm table-bordered" style="font-size:13px;text-align:left">
          <thead><tr><th style="font-weight:normal">Repuesto</th><th style="font-weight:normal">Cant.</th><th style="font-weight:normal">C.C.</th><th style="font-weight:normal">Urgencia</th><th style="font-weight:normal">Grupo</th><th style="font-weight:normal">Descripción</th><th style="font-weight:normal">Estado</th></tr></thead>
          <tbody>${filas}</tbody>
        </table></div>`,
      width: 750,
  ...
  const badgeEstado = (e) => {
    ...
    const color = { 'En analisis': 'primary', Pedido: 'primary', Pendiente: 'secondary', 'En proceso': 'warning', Completado: 'success', Cancelado: 'danger' }
    const label = e === 'Pedido' ? 'En analisis' : e
    return <span className={`badge bg-${color[e] || 'secondary'}`}>{label}</span>
  }
                ...
                <div className="modal-body">
                  ...
                  <div className="row mb-3">
                    ...
                    <div className="col">
                      ...
                      <input className="form-control" value={form.estado === 'Pedido' ? 'En analisis' : form.estado} readOnly style={{ backgroundColor: '#f8f9fa', cursor: 'default' }} />
                    </div>
                  </div>
                </div>
